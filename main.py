...
ADMINS = [7868124597] # የእርስዎ ID እዚህ ተስተካክሏል
...
def create_ticket_image(name, ticket, car, phone, reg_code, sale_date):
    ...
    d.rectangle([50, 50, 1950, 1350], outline=(212, 175, 55), width=60) 
    
    d.text((500, 100), "ግዛቸው የመኪና እቁብ", fill=(212, 175, 55))
...
async def screenshot_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_text("እጣዎ በሚገባ ተመዝግቧል! ክፍያው እስከሚረጋገጥ ትንሽ ይጠብቁ...", reply_markup=ReplyKeyboardRemove())
...
async def handle_v(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if act == "v":
        ...
        if u:
            ...
            await context.bot.send_photo(uid, img, caption="በተሳካ ሁኔታ ተመዝግበዋል!\n🎉 መልካም እድል 🎉", 
                                       reply_markup=ReplyKeyboardMarkup([["ሌላ እጣ ይቁረጡ"]], resize_keyboard=True))
            await q.edit_message_caption(q.message.caption + "\n\n✅ ተረጋግጧል!")
    else: await q.edit_message_caption(q.message.caption + "\n\n❌ ተሰርዟል!")
...
async def broadcast_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in users:
        try:
            ...
            elif update.message.video: await context.bot.send_video(u[0], update.message.video.file_id, caption=update.message.caption)
        except: pass
...
if __name__ == '__main__':
    ...
    application.add_handler(ConversationHandler(
        ...
        states={
            ...
            BROADCAST: [MessageHandler((filters.TEXT | filters.PHOTO | filters.VIDEO) & ~filters.COMMAND, broadcast_handler)]
        },
