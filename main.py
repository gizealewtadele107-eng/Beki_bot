...
app = Flask('')
@app.route('/')
def home(): return "Bot is Secured and Online!"
...
# --- 2. ዋና ቅንብሮች (የተጠበቁ) ---
# አዲሱ Token ከፎቶው ላይ የተወሰደ
TOKEN = "8667033966:AAFCDyp-2FSr_JNlluB8YwhRJXLpTlCH1Ts"
# አድሚን ID (7868124597)
ADMINS = [7868124597] 
...
async def ask_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ ጥያቄዎ ለአድሚን ተልኳል! በቅርቡ መልስ ይሰጥዎታል በ /start ይመለሱ።")
...
async def ticket_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if check:
        await update.message.reply_text("❌ ይህ ቲኬት ተይዟል! ሌላ ቁጥር ይምረጡ፦")
        return TICKET
...
async def screenshot_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_text("⌛ ተመዝግቧል! አድሚኑ እስኪያረጋግጥ ድረስ ትንሽ ይጠብቁ...", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
    return SCREENSHOT
