...
# --- ዋና ቅንብሮች ---
# አዲሱ ቶክን እና ሁለቱ አድሚኖች እዚህ ተካተዋል
...
def create_ticket_image(name, ticket, car, phone, reg_code, sale_date):
    ...
    d.text((70, 310), f"🔑 የመዝገብ ቁጥር: {reg_code}", fill=(255, 255, 255))
...
async def broadcast_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("ለተጠቃሚዎች የሚላክ ፎቶ ከነ ጽሁፉ (Caption) ይላኩ፦")
...
async def send_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_text("✅ መልእክቱ ለሁሉም ተልኳል!")
...
async def car_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "📣 ለሁሉም መልእክት ላክ" and update.effective_user.id in ADMINS:
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(update.message.text.split()) < 3:
        await update.message.reply_text("⚠️ ስምዎ ባለ 3 ቃል መሆን አለበት። እንደገና ያስገቡ፦")
        return NAME
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (text.startswith('0') and len(text) == 10 and text.isdigit()):
        await update.message.reply_text("⚠️ ስልክ ቁጥር 10 አሃዝ መሆን አለበት። እንደገና ያስገቡ፦")
        return PHONE
...
async def get_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_text("✅ መረጃዎ ደርሶናል። አስተዳዳሪው ሲያረጋግጥ ቲኬቱ ይላክልዎታል!", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
    return SCREENSHOT
